...
const all = async function(){
    //console.log('总回调启动..')
            chrome.storage.local.get(["my_rate",'color','lazada_nodes','lazada_main_price_nodes'],async function(result){
...
let throttle_callback = _.throttle(callback,2700,false);
...
const insetHtml=function(a){
    ...
    for(let i =0;i<a_length;i++){
        ...
        if(rmb != 'NaN'){
            a[i].innerHTML=s + `<sub translate="no" style="color:${color}" title="¥${rmb}"> ¥${rmb}</sub>`;
       ...
       //console.log('有详情页主价格节点...')
        ...
        if(node.nextSibling&&node.nextSibling.innerText.includes('¥')){
            return
                ...
                a.style.color = color
                a.setAttribute('style',`color:${color};font-size:20px`)
                a.id = 'knipfly_my'
                a.innerText = '  ¥' + rmb
