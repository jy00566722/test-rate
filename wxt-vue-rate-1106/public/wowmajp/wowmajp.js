console.log('这里是 wowma.jp 价格转换插件,启动!')
...
const all=function(){
    //console.log('总回调启动');
    chrome.storage.local.get(["my_rate",'wowmajp_nodes','color'],function(result){
        rate = result.my_rate[`rate_${country}`];
        wowmajp_nodes = result.wowmajp_nodes
        if(result.color){
            color = result.color
        }
        foreach_nodes(wowmajp_nodes)
    })
}
